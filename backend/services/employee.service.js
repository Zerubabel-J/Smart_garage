...
async function editEmployee(id, employee) {
    ...
    try {
        ...
        await query(
            "UPDATE employee_role SET company_role_id = ? WHERE employee_id = ?", [employee.company_role_id, employee.employee_id]
        );

        // If the password is provided, update the employee_pass table
        if (employee.employee_password) {
            ...
            const hashedPassword = await bcrypt.hash(employee.employee_password, salt);
            await query(
                "UPDATE employee_pass SET employee_password_hashed = ? WHERE employee_id = ?", [hashedPassword, employee.employee_id]
            );
        }
